...
describe("scan-period", () => {
  ...
  it("parseScanPeriod는 유효값만 통과시키고 나머지는 기본값으로 정규화한다", () => {
    ...
    expect(parseScanPeriod("365d")).toBe("365d");
    expect(parseScanPeriod("all")).toBe(SCAN_PERIOD_DEFAULT); // 더 이상 유효값 아님
  ...
  it("라벨을 한국어로 돌려준다", () => {
    ...
    expect(scanPeriodLabel("90d")).toBe("최근 3개월");
  });
});
